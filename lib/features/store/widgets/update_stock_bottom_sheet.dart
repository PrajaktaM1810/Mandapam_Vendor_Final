...
class _UpdateStockBottomSheetState extends State<UpdateStockBottomSheet> {
  ...
  void _setTotalStock() {
    ...
    for (TextEditingController stockController in variationStockControllers) {
      totalStock += int.tryParse(stockController.text.trim()) ?? 0;
    }
    mainStockController.text = totalStock.toString();
    setState(() {});
  }
              ...
              child: GetBuilder<StoreController>(
                  builder: (storeController) {
                    return !storeController.isLoading ? CustomButtonWidget(
                      ...
                      onPressed: () {
                        if(mainStockController.text.isEmpty || mainStockController.text == '0'
                            || variationStockControllers.any((element) => element.text.isEmpty || element.text == '0')) {
  ...
  void _updateStock(StoreController storeController) {
    ...
    final variations = widget.item.variations ?? [];
    for (var variation in variations) {
      final index = variations.indexOf(variation);
      if(index >= 0 && index < priceControllers.length) {
        data.addAll({
          "price_${index}_${variation.type ?? ''}": priceControllers[index].text.trim(),
          "stock_${index}_${variation.type ?? ''}": variationStockControllers[index].text.trim(),
        });
      }
    }

    final types = variations.map((v) => v.type ?? '').toList();
    data.addAll({"type": jsonEncode(types)});

    storeController.stockUpdate(data, widget.item.id ?? 0).then((isSuccess) {
